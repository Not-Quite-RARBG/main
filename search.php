...
<body style="background-color:black;padding:30px;font-family:monospace;">
  ...
  <div style="text-align:center;position:relative;">
    <div style="background-color:white;display:inline-block;border-radius:5px;padding:15px;position:relative;">
      ...
      $jsonData = json_decode(curl_get_contents($api_url."search.php?"."&api_key=$api_key"."&page=$page&query=".urlencode($_GET['q'])))->data;
      ?>
      <table width="100%">
    <tbody>
      <tr>
        <td colspan="2" align="center">   
          <h1 class="black"><?php echo $_GET['q'];?></h1>
          <div align="center">
            <div id="pager_links">
              Pages: 
              <?php
              $pages = json_decode(curl_get_contents($api_url."search.php?"."&api_key=$api_key"."page=$page&query=".urlencode($_GET['q'])."&api_key=$api_key"))->pages;
                // Loop through the pages and generate a link for each one
                for ($i = 1; $i <= $pages; $i++) {
                  $url = "torrents.php?q=" . urlencode($_GET['q']) . "&page=" . $i;
            ...
            echo '<tr>
              ...
              <td>
                <div style="text-align:left;">
                  <a href="/torrent/' . $item->ext_id . '" style="font-weight:900;">' . $item->title . '</a>
                </div>
              </td>
              <td>
